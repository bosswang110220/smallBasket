(function($) {
	$.fn.imgZoom = function() {
		...
		$(this).unbind().click(function() {
			var mask =
				"<div style = 'position: fixed;width: 100%;z-index: 5555;height: 100%;top: 0;left: 0;background: rgba(0,0,0,0.5);' id='imgZoomMask' onclick='maskClose()'></div>";
			$("body").append(mask);
			var windowWidth = $(window).width();
			var windowHeight = $(window).height();
			$(window).resize(function() {
			...
			img.onload = function() {
				...
				if(img.width > img.height) {
					...
					dom = "<img draggable='true' src = '" +
						...
						"' width = '50%' style='" +
						style +
						"' id='imgZoomImg' onclick='zIndexMask()'>";
				} else {
